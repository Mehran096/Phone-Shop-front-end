const HeroBanner = () => {
  return (
    <>
    <div className="w-screen -mx-4 lg:-mx-0 relative bg-gradient-to-b from-black via-gray-900 to-gray-700 text-white">
      <div className="max-w-6xl mx-auto px-4 py-16 lg:py-24 flex-col items-center text-center">
        
        {/* Title */}
        <h1 className="text-3xl sm:text-4xl md:text-6xl font-bold mb-3 px-2">
          PhoneStore Pro Max
        </h1>

        {/* Subtitle - breaks nicely on mobile */}
        <p className="text-base sm:text-lg md:text-xl text-gray-300 mb-6 px-4 leading-relaxed">
          Snapdragon® 8 Elite <span className="hidden sm:inline">|</span>
          <br className="sm:hidden" />
          Ultra-Slim Design <span className="hidden sm:inline">|</span>
          <br className="sm:hidden" />
          6000 mAh Battery
        </p>
        
        {/* Button */}
        <button className="bg-white text-black px-6 py-3 rounded-full font-medium hover:bg-gray-200 transition">
          Learn more
        </button>

        {/* Product Image */}
        <img 
          src="/assets/HeroBanner.png" 
          alt="Phones"
          className="mt-10 w-full max-w-6xl md:max-w-7xl mx-auto object-contain"
        />
      </div>
      
    </div>
     
    </>
    
  )
}

export default HeroBanner