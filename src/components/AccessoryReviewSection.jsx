...
const AccessoryReviewSection = ({ accessory }) => {
 ...
 const cancelEdit = () => {
  ...
  className='text-red-600 text-xs flex items-center gap-1 disabled:opacity-50 disabled:cursor-not-allowed'
>
  {loadingDelete? (
    <>
        ...
        <div className='max-w-2xl bg-white p-6 rounded-lg border mt-8'>
          ...
          <button
            ...
            className={`w-full py-3 rounded-lg font-semibold text-white transition-all duration-200 flex items-center justify-center gap-2
              ${
                // 1. Loading state - gray
                isSubmitting || loadingReview
                  ? 'bg-gray-400 cursor-not-allowed'
                  // 2. Invalid state - light gray
                  : rating === 0 || !comment
                  ? 'bg-gray-300 text-gray-600 cursor-not-allowed'
                  // 3. Valid state - black gradient
