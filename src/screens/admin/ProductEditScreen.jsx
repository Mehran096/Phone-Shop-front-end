...
const ProductEditScreen = () => {
  ...
  const addColorHandler = () => {
    setColors([...colors, { name: '', hexCode: '', countInStock: 0, price: '', images: [], imagePublicIds: [], files: [], filePreviews: [] }])
  }
          ...
          <form onSubmit={submitHandler} className='space-y-4'>
            ...
            <div className='border-t pt-4'>
              <div className='flex justify-between items-center mb-3'>
                ...
                <button type='button' onClick={addSpecHandler} className='px-3 py-1 bg-green-600 text-white rounded hover:bg-green-700 text-sm flex items-center gap-1'>
                  <FaPlus /> Add Spec
                </button>
              </div>
              {specs.map((spec, index) => (
                <div key={index} className='flex gap-2 mb-2'>
                  <input type='text' placeholder='Key' value={spec.key} onChange={(e) => handleSpecChange(index, 'key', e.target.value)} className={inputClass} />
                  <input type='text' placeholder='Value' value={spec.value} onChange={(e) => handleSpecChange(index, 'value', e.target.value)} className={inputClass} />
                  {specs.length > 1 && (
                    <button type='button' onClick={() => removeSpecHandler(index)} className='px-3 text-red-600 hover:text-red-800'>
                      <FaTrash />
                    </button>
                  )}
                </div>
